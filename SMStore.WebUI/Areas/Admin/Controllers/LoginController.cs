...
namespace SMStore.WebUI.Areas.Admin.Controllers
{
    ...
    public class LoginController : Controller
    {
        ...
        private readonly IValidator<AdminLoginViewModel> _validator; // FluentValidation ı kullanmak için ekliyoruz
        ...
        public async Task<IActionResult> IndexAsync(AdminLoginViewModel model)
        {
            ...
            if (result.IsValid) //FluentValidation kontrolü sonucu geçerliyse 
            {
                try
                {
            ...
            else
            {
                foreach (var error in result.Errors) // Eğer validasyon başarısızsa oluşan hataları result.Errors ile yakalayıp foreach döngüsüyle ekrana yollayabiliriz
                {
                    ...
                    ModelState.AddModelError("", error.ErrorMessage); // FluentValidation ın hata mesajlarını ekranda göstermek için
                }
            }
            return View();
        }
